...
export function fnCreateLogger(): winston.Logger {
  const LLogsDir = path.resolve(process.cwd(), "logs");
  fs.mkdirSync(LLogsDir, { recursive: true });
  ...
  return winston.createLogger({
    ...
    format: winston.format.combine(
      ...
      winston.format.printf(({ timestamp: LTimestamp, level: LLevel, message: LMessage, stack: LStack }) => {
        return `${LTimestamp} ${LLevel}: ${LStack ?? LMessage}`;
      })
    ),
    transports: [
      ...
      new winston.transports.File({
        filename: path.join(LLogsDir, `cleanup-${fnDateStamp()}.log`)
      })
    ]
  });
}
